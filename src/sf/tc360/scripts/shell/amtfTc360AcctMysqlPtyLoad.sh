...
###########################################################
# Script Name: amtf_Tc360AcctMysqlPtyLoad.sh              #
...
spark-submit ${deploy_mode_tc360_attributes} ${spark_resources_tc360_attributes} ${python_spark_dir}/amtfTc360AcctMysqlPtyLoad.py -f ${config_file_dir}/${config_file} -h ${mysql_amf_pty_hostname} -d ${mysql_amf_pty_dbname} -n ${mysql_amf_pty_jdbc_port} -u ${mysql_amf_pty_user_id} -p ${mysql_amf_pty_pass} -m ${mysql_script_dir} -j ${mysql_amf_pty_jceks} -r ${runDay} >> ${log_file_dir}/amf_service_c360_attr_mysql_pty_load_${log_time}.log
...
if [ $? -ne 0 ]
 then 
    ...
    send_email -t "${recipient_tc360_mysql_pty}" -c "${cc_tc360_mysql_pty}" -s "${subject_tc360_mysql_pty_fail}" -b "${body_tc360_mysql_pty_fail}" -a "${log_file_dir}/amf_service_c360_attr_mysql_pty_load_${log_time}.log"
    if [ $? -eq 0 ]
        then
    ...
    send_email -t "${recipient_tc360_mysql_pty}" -c "${cc_tc360_mysql_pty}" -s "${subject_tc360_mysql_pty_success}" -b "${body_tc360_mysql_pty_success}" -a "${log_file_dir}/amf_service_c360_attr_mysql_pty_load_${log_time}.log"
    if [ $? -eq 0 ]
        then
         echo "Email notification send successfully" >> ${log_file_dir}/amf_service_c360_attr_mysql_pty_load_${log_time}.log
    else 
