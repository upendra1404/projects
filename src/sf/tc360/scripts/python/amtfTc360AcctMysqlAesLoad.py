###################################################################
# Purpose: Loading Tc360 aci attribute into MYSQL aes table       #
...
def createSparkSession(protegrityJar,mySQLJarFile,mySQLAesJar,sparkJar):
 spark = SparkSession.builder.\
                     enableHiveSupport().appName('Tc360MySQLAesApp').getOrCreate()
...
def readTc360Account(extPathAcct,runDay,spark):
 try:
  print("#########################################################################################################################")
  ...
  tc360AcctDf = tc360AcctDf.withColumn('account_number',explode(split(regexp_replace(col("account_number"), "[^0-9A-Za-z,]", ""),","))).withColumn('account_number',expr("ptyUnprotectStr(account_number,'dtStrNPL')")).withColumn('account_number',expr("encrypt(account_number,'ACCOUNT_NUMBER')")).dropDuplicates()
...
def createLRDf(tc360AcctDf):
 try:
  ...
  mysqlAesDf = tc360AcctDf.select(col('account_number').cast('varchar(100)').alias('account_number')
  ...
  sys.exit("ERROR: while creating mysql aes load ready df:" + str(e))

def truncateaesMySql(sqlParams,mysqlAmfScriptPath):
 try:
  print("####################Truncating mysql aes table ####################")
  ...
  sys.exit("ERROR: while truncating the mysql aes table:" + str(e))
  
 ## Loading mysql dlz table with protegrity tokenisation
def load_tc360_mysql_aes(mysqlAesDf,jdbcUrl,mySQLDriver,mysqlTc360Tbl,mysqlAmfUserId,mysqlAmfPassword):
 try:  
     print("Loading C360 attribute MYSQL aes table: Starts")              
     mysqlAesDf.write.format('jdbc').\
                     options(
                             url=jdbcUrl,
                             driver=mySQLDriver,
                             ...
                             mode('overwrite').\
                             save()
     print("Total record loaded into mysql aes table is: "+ str(mysqlAesDf.count()))
     print("Loading C360 attribute mysql aes table: Complete")
     print("###########################################################################################################################")
 except Exception as e:
  sys.exit("ERROR: while loading mysql aes table:" + str(e))
...
def main():
 ...
 tc360AcctDf = readTc360Account(inputParams['extPathAcct'],runDay,spark)
 mysqlAesDf = createLRDf(tc360AcctDf)
 truncateaesMySql(sqlParams,configParams['mysqlAmfScriptPath'])
 load_tc360_mysql_aes(mysqlAesDf,jdbcUrl,inputParams['mySQLDriver'],inputParams['mysqlTc360Tbl'],sqlParams['mysqlAmfUserId'],sqlParams['mysqlAmfPass'])
