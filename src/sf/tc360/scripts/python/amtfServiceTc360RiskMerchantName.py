########################################################################################
# Script Name:amtf_tc360_risk_merchant_name.py                                         #
# Purpose: To compute risk merchant name attributes and load those into EDL hive table #
    ...
    if(not(os.path.exists(configFileName))):
        ...
        if('SCHEMA' not in config.sections()):
            ...
            sys.exit("ERROR: The Configuration file doesn't have section: HDFSPATH")
                ...
                if ('tc360_risk_mrch_nm_hive' not in config.options('HIVE')):
                  sys.exit("ERROR: Section HIVE doesn't contain tc360_risk_mrch_nm_hive")
                else:
                    tc360_risk_mrch_nm_hive = config.get('HIVE','tc360_risk_mrch_nm_hive')
        ...
        return {
                            ...
                            ,'tc360_risk_mrch_nm_hive': tc360_risk_mrch_nm_hive
                            ,'ftccPath': ftccPath
                            ,'ftctPath': ftctPath
                            ,'runDay': runDay
                            ,'debug': debug
...
def createSparkSession():
 spark = SparkSession.builder.\
                enableHiveSupport().appName('tc360_risk_merchant_name').getOrCreate()
...
def createLoadReadyDf(riskAttrDf,runDay):
 ...
 #################################################################
 try:   
  cols = ['merchant_name']+['auth_1d','auth_7d','auth_30d','auth_60d','auth_90d','auth_amt_1d','auth_amt_7d','auth_amt_30d','auth_amt_60d','auth_amt_90d']+['fraud_1d','fraud_7d','fraud_30d','fraud_60d','fraud_90d','fraud_amt_1d','fraud_amt_7d','fraud_amt_30d','fraud_amt_60d','fraud_amt_90d']
...
#######################################################################################################################
def loadC360AttributeHive(riskMerchAttrDf,tc360_risk_mrch_nm_hive,writeMode,spark):
 try:
  print("######################## Starting loading tc360 risk merchant name table ######################################")              
  spark.sql("set hive.exec.dynamic.partition=true")
  spark.sql("set hive.exec.dynamic.partition.mode=nonstrict")
  riskMerchAttrDf.write.mode(writeMode).insertInto(tc360_risk_mrch_nm_hive,overwrite=True)
  print("######################## tc360 risk merchant name hive table load complete ###########################################")
 except Exception as e:
...
def main():
 ...
 riskFalconAttrDf = riskModule.computeRiskFalconAttributes(falconDf,'ffsl_mrch_nm',runDay)
 fccAggDf = riskModule.createFccAggDf(fccDf,fctDf)
 riskFccAttrDf = riskModule.computeRiskFCCAttributes(fccAggDf,'ffsl_mrch_nm',runDay)
 riskAttrDf = riskModule.combinedDf(riskFalconAttrDf,riskFccAttrDf,'ffsl_mrch_nm')
 riskMerchAttrDf = createLoadReadyDf(riskAttrDf,runDay)
 loadC360AttributeHive(riskMerchAttrDf,inputParams['tc360_risk_mrch_nm_hive'],inputParams['writeMode'],spark)
