###############################################################################
# Script Name:amtfServiceTc360TimeBoxedAttrLoad.py                            #
# Purpose: To calculate time boxed attributes and load into EDL               #
        ...
        if('SCHEMA' not in config.sections()):
            ...
            sys.exit("ERROR: The Configuration file doesn't have section: HDFSPATH")
                ...
                if ('extPathTBAttr' not in config.options('HDFSPATH')):
                  sys.exit("ERROR: Section HDFSPATH doesn't contain extPathTBAttr")
                else:
...
def createSparkSession():
 spark = SparkSession.builder\
                     .enableHiveSupport().appName('tc360_timeboxed_attr_apps').getOrCreate()
...
def computeTimeBoxedStddev(falconAggDf,n,runDay):
 ...
 try:
  ...
  tbStdDevAttrDf = falconAggDf.filter("ffsl_fraud_trancode in ('MA','CA','AP')")\
                         ...
                         .agg(count('tran_amount').alias('countTxn'),_sum('tran_amount').cast('double').alias('sumTxnAmt'))\
...
def computeTopPOSAttr(falconAggDf,runDay,n):
 try:
  ...
  sys.exit("ERROR: while computing top pos      -" + str(e))
...
def readPrecursorTables(pcAcctLexid,edl_amf_falcon_raw,falconRawPath,debug,runDay,spark):
 try:
  ...
  accXref = lexidDf['orig_account_nbr','current_account_nbr','customer_acct_nbr','acct_bin'].dropDuplicates() # Creating accXref dataframe to compute historical statistics
...
def computeAuthEAttr(eAuth979AggDf,eAuth323AggDf,eAuth257AggDf,runDay):
 ...
 try:
  ...
  df = eAuth979AttrDf.join(eAuth323AttrDf,'orig_account_nbr','full')
  eAuthAttrDf = df.join(eAuth257AttrDf,'orig_account_nbr','full')
  eAuthAttrDf.cache()
  return eAuthAttrDf
 except Exception as e:
...
def computeFccBin6Attributes(fccDf,runDay):
 try:
  fccDf = fccDf.withColumn('bin6',substring(expr("ptyUnprotectStr(current_account_nbr_pty,'dtStrNPL')"),1,6))
  fccBin6AttrDf = fccDf.groupBy('bin6')\
                       .agg(countDistinct(expr("case when trim(loss_type_cd) <> '03' and datediff(to_date('" + runDay + "'),reported_date)<=1 then case_id else 0 end")).alias('acct_fraud_claim_non_tnf_bin6_1d')\
  ...
  return fccBin6AttrDf
 except Exception as e:
  sys.exit("ERROR: while computing timeboxed fcc attributes - " + str(e))

def computeDerivedFalconAttr(falconAttrDf):
 try:
  ...
  falconAttrDf = falconAttrDf.drop(*['avg_bw_7_14_days','avg_bw_30_60_days'])
  return falconAttrDf
 except Exception as e:
  sys.exit("ERROR: while deriving falcon additional attributes - " + str(e))
  
  
def combinedDf(falconAttrDf,eAuthAttrDf,fccBin6AttrDf,bin6FalconAttr6Df,nmAttrDf,accXref):
 # Combining all the data frames
 try:
  timeBoxAttrDf = falconAttrDf.join(eAuthAttrDf,['orig_account_nbr'],'left')
  timeBoxAttrDf = timeBoxAttrDf.join(nmAttrDf,['orig_account_nbr'],'left')
  timeBoxAttrDf = timeBoxAttrDf.join(accXref,['orig_account_nbr'],'left')
  timeBoxAttrDf = timeBoxAttrDf.join(fccBin6AttrDf,fccBin6AttrDf['bin6']==timeBoxAttrDf['acct_bin'],how = 'left')
  timeBoxAttrDf = timeBoxAttrDf.join(bin6FalconAttr6Df,bin6FalconAttr6Df['bin6']==timeBoxAttrDf['acct_bin'],how = 'left')
  timeBoxAttrDf = timeBoxAttrDf.drop(*['bin6','current_account_nbr','current_account_nbr_pty','acct_bin']).dropDuplicates()
  
  return timeBoxAttrDf
 except Exception as e:
  sys.exit("ERROR: while combining all data frames -" + str(e))
...
def readFalconBin6Attributes(extFalconBin6Path,runDay,spark):
 try:
  ...
  bin6FalconAttr6Df = spark.read.parquet(extFalconBin6Path).where(col('as_of_date') == lit(runDay)).drop(*['as_of_date','edl_load_ts']).toDF(*colsBin6Attr)
...
def createLoadReadyDf(tc360TimeBoxAttrDF,runDay):
 try:
  ##Forming final tc360 account level attribute data frame
  tc360FalconAttrDF = tc360TimeBoxAttrDF.dropDuplicates().withColumn('edl_load_ts',lit(current_timestamp())).withColumn('as_of_date',lit(runDay))
  return tc360FalconAttrDF
 except Exception as e:
...
#######################################################################################################################
def loadTimeBoxedAttr(tc360FalconAttrDF,extPathTBAttr,writeMode,spark):
 try:
  print("######################## Started loading tc360 timeboxed attribute hive table ######################################")              
  spark.sql("set hive.exec.dynamic.partition=true")
  spark.sql("set hive.exec.dynamic.partition.mode=nonstrict")
  tc360FalconAttrDF.write.mode(writeMode).option('header','true').format('parquet').partitionBy('as_of_date').save(extPathTBAttr)
  print("######################## Tc360 timeboxed attribute hive table load complete ###########################################")
 except Exception as e:
...
def main():
 ...
 lexidDf,readFalconDf,accXref = readPrecursorTables(inputParams['pcAcctLexid'],inputParams['edl_amf_falcon_raw'],inputParams['falconRawPath'],inputParams['debug'],runDay,spark)
 ...
 falconAttrDf = computeDerivedFalconAttr(falconAttrDf)
 fccBin6AttrDf = computeFccBin6Attributes(fccDf,runDay)
 timeBoxAttrDf = combinedDf(falconAttrDf,eAuthAttrDf,fccBin6AttrDf,bin6FalconAttr6Df,nmAttrDf,accXref)
 tc360TimeBoxAttrDF = createLoadReadyDf(timeBoxAttrDf,runDay)
 loadTimeBoxedAttr(tc360TimeBoxAttrDF,inputParams['extPathTBAttr'],inputParams['writeMode'],spark)
