###################################################################################
# Script Name:amtf_tc360_risk_bin6.py                                            #
# Purpose: To compute risk bin6 attributes and load those into EDL hive table    #
    ...
    if(not(os.path.exists(configFileName))):
        ...
        if('SCHEMA' not in config.sections()):
            ...
            sys.exit("ERROR: The Configuration file doesn't have section: HDFSPATH")
                ...
                if ('tc360_risk_bin6_hive' not in config.options('HIVE')):
                  sys.exit("ERROR: Section HIVE doesn't contain tc360_risk_bin6_hive")
                else:
                    tc360_risk_bin6_hive = config.get('HIVE','tc360_risk_bin6_hive')
                ...
                if ('extFalconBin6Path' not in config.options('HDFSPATH')):
                  sys.exit("ERROR: Section HIVE doesn't contain extFalconBin6Path")
                else:
        ...
        return {
                            ...
                            ,'tc360_risk_bin6_hive': tc360_risk_bin6_hive
                            ,'ftccPath': ftccPath
                            ,'ftctPath': ftctPath
...
def createSparkSession():
 spark = SparkSession.builder.\
                enableHiveSupport().appName('tc360_risk_bin6_application').getOrCreate()
...
def createLoadReadyDf(riskAttrDf,bin6AttrDf,runDay):
 #################################################################
 # Forming final tc360 account level attributes data frame       #
 # Forming cols as per the risk merchant name table requirements #
 #################################################################
 try:   
  cols = ['bin6','merchant_id']+['auth_1d','auth_7d','auth_30d','auth_60d','auth_90d','auth_amt_1d','auth_amt_7d','auth_amt_30d','auth_amt_60d','auth_amt_90d']+['fraud_1d','fraud_7d','fraud_30d','fraud_60d','fraud_90d','fraud_amt_1d','fraud_amt_7d','fraud_amt_30d','fraud_amt_60d','fraud_amt_90d']
  ...
  bin6AttrDf = bin6AttrDf.withColumn('edl_load_ts',lit(current_timestamp())).withColumn('as_of_date',lit(runDay))
  
  bin6AttrDf.cache()
  riskMerchAttrDf.cache()
  return riskMerchAttrDf,bin6AttrDf
 except Exception as e:
  sys.exit("ERROR: while forming the final risk merchant dataframe - " + str(e))

#######################################################################################################################
# Function for loading the C360 risk bin 6 attributes hive table                                                      #
#######################################################################################################################
def loadC360AttributeHive(riskMerchAttrDf,bin6AttrDf,tc360_risk_bin6_hive,extFalconBin6Path,writeMode,spark):
 try:
  print("######################## Starting loading tc360 risk bin6 table ######################################")              
  spark.sql("set hive.exec.dynamic.partition=true")
  spark.sql("set hive.exec.dynamic.partition.mode=nonstrict")
  riskMerchAttrDf.write.mode(writeMode).insertInto(tc360_risk_bin6_hive,overwrite=True)
  bin6AttrDf.write.mode(writeMode).option('header','true').partitionBy('as_of_date').parquet(extFalconBin6Path)
  print("######################## tc360 risk merchant hive table load complete ###########################################")
 except Exception as e:
  sys.exit("ERROR: while loading the data into risk bin6 hive table - " + str(e))
...
def main():
 ...
 # Reading falcon for bin6 attributes
 falconBin6Df = riskModule.readFalconBin6(inputParams['falconRawPath'],inputParams['edl_amf_falcon_raw'],runDay,90,inputParams['debug'],spark)
 # Reading falcon for bin6 merchant id related attributes
 ...
 # creating risk falcon attributes for key bin6 and ffsl_frad_mrch_id 
 riskFalconAttrDf = riskModule.computeRiskFalconAttributes(falconDf,['bin6','ffsl_frad_mrch_id'],runDay)
 # creating bin6 sales attributes df
 bin6AttrDf = riskModule.computeRiskFalconAttributes(falconBin6Df,'bin6',runDay)
 fccAggDf = riskModule.createFccAggDf(fccDf,fctDf)
 riskFccAttrDf = riskModule.computeRiskFCCAttributes(fccAggDf,['bin6','ffsl_frad_mrch_id'],runDay)
 riskAttrDf = riskModule.combinedDf(riskFalconAttrDf,riskFccAttrDf,['bin6','ffsl_frad_mrch_id'])
 riskMerchAttrDf,bin6AttrDf = createLoadReadyDf(riskAttrDf,bin6AttrDf,runDay)
 loadC360AttributeHive(riskMerchAttrDf,bin6AttrDf,inputParams['tc360_risk_bin6_hive'],inputParams['extFalconBin6Path'],inputParams['writeMode'],spark)
